...
loc = json.load(open(f"Core/music_en.json","r",encoding="utf-8"))
# Silence useless bug reports messages
youtube_dl.utils.bug_reports_message = lambda: ''

Audioconfig = ArusuConfig()
...
async def checkUserAndBotChannel(ctx):
    if isBotInChannel(ctx):
        ...
        await respond(ctx,loc["messages"]["not_connected"])
        return False
...
async def _join(ctx, from_cmd=False, author=None):
    ...
    if isBotInChannel(ctx):
        await respond(ctx, loc["messages"]["already_in_voice"])
        return False
    ...
    try:
        ...
        await respond(ctx, loc["messages"]["cannot_join_unk"])
        return False
    ...
    if isinstance(ctx.voice_state.voice.channel, discord.StageChannel):
        try:
            ...
            await respond(ctx, loc["messages"]["require_mute"])
...
class SearchMenu(discord.ui.Select):
    ...
    async def callback(self, interaction):
        ...
        if not self.ctx.author.voice or not self.ctx.author.voice.channel:
            return await respond(interaction.message, loc["messages"]["enter_before_play"])
        ...
        if voice_client:
            if voice_client.channel != self.ctx.author.voice.channel:
                return await respond(interaction.message, loc["messages"]["already_in_voice"])
...
class Music(commands.Cog):
    ...
    async def volume(self, ctx, volume=None):
        ...
        if volume is not None:
            ...
            if 0 > volume or volume > 200:
                return await respond(ctx, loc["messages"]["volume_invalid_range"])
            # The volume should be divided by 100
            ctx.voice_state.volume = volume / 100
            await respond(ctx, loc["messages"]["updated_volume"].format(volume))
        else:
            # Return the current volume
            return await respond(ctx, loc["messages"]["current_volume"].format(int(ctx.voice_state.volume*100)))
    ...
    async def playfile(self, ctx, *, title=None):
        ...
        try:
            ...
            return await respond(ctx, loc["messages"]["cannot_add_invalid_file"])
        # Displaying filename with _ will cause discord to format the text, replace them with \_ to avoid this problem
        await respond(ctx, loc["messages"]["added_song"].format(title.replace("_", "\\_")))
        ctx.voice_state.stopped = False
    ...
    async def seek(self, ctx, seconds=None):
        ...
        if ctx.voice_state.is_playing and ctx.voice_state.voice.is_playing():
            ...
            try:
                ...
                return await respond(ctx, loc["messages"]["parse_second_failed"])
            if seconds is None:
